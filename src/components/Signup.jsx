...
const Signup = () => {
  ...
  const captchaGenerator = () => {
    ...
    setState((prev) => {
      return { ...prev, captcha: uniqueChar };
    });
  };
      ...
      <>
        <div className="container container--green">
          <div className="form">
            ...
            {Object.entries(state.form).map(([key, val],index) => (
              <p key={`complete${index}`}>{`${key.replace("_", " ")} : ${val.value}`}</p>
            ))}
            <button
              className="btn"
              onClick={() => {
                setTimeout(() => captchaGenerator(), 300);
                setState({...state,signupCompleted:false,captchaVerified:false });
              }}
            >
              Go Back
            </button>
    ...
    <>
      <div className={`container ${darkMode ? "container--dark" : ""}`}>
        ...
        >
          ...
          {Object.entries(state.form).map(([key, val],index) => (
            <>
              <div key={`incomplete${index}`} className="input-wrapper">
